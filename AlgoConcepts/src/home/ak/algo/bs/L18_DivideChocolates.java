 ...
 * 
 *         Example 1: Input: sweetness = [1,2,3,4,5,6,7,8,9], K = 5 Output: 6
 *         Explanation: You can divide the chocolate to [1,2,3], [4,5], [6],
 *         [7], [8], [9]
 * 
 *         Example 2: Input: sweetness = [5,6,7,8,9,1,2,3,4], K = 8 Output: 1
 *         Explanation: There is only one way to cut the bar into 9 pieces.
 * 
 *         Example 3: Input: sweetness = [1,2,2,1,2,2,1,2,2], K = 2 Output: 5
 *         Explanation: You can divide the chocolate to [1,2,2], [1,2,2],
 *         [1,2,2]
 * 
 *         Constraints:
 * 
 ...
 *
 */
public class L18_DivideChocolates {

	public static int maximizeSweetness(int[] sweetness, int K) {
		int totalChunks = K + 1; // friends + me
		int low = 1; // minimum sweetness
		int high = Arrays.stream(sweetness).sum() / totalChunks; // maximum sweetness we can have

		while (low < high) {
			int mid = low + (high - low + 1) / 2; // to avoid infinite loop on same mid value

			if (isPossibleToCut(sweetness, K, mid)) {
				// reduce the chunks
				low = mid;
			} else {
				high = mid - 1;
	...
	private static boolean isPossibleToCut(int[] sweetness, int K, int minSweetness) {
		int chunks = 0;
		int sum = 0;
		for (int i = 0; i < sweetness.length; i++) {
			sum += sweetness[i];
			if (sum >= minSweetness) { // If the sweetness is more than minimum, slice it
				chunks++; // cut the cake
				sum = 0;
			}
		}
